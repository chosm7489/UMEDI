...
function addMedInfo() {
  ...
  firebase.auth().onAuthStateChanged((user) => {
    if (user)
      ...
      db.collection("users").doc(user.uid).collection("medications")
        .add({
          name: Name,
          code: Code,
          intake: Intake,
...
function updateMedImage(userid, medid) {
  ...
  storageRef
    .put(ImageFile) //picture that was chosen
    .then(function () {
      //Asynch call to get URL from Cloud
      storageRef
        .getDownloadURL()
        .then((url) => {
          //"url" is the returned url pointing to cloud image
          //Asynch call to image URL the form fields into Firestore.
          db.collection("users").doc(userid).collection("medications").doc(medid).update({picture: url, // Save the URL into users collection
            });
        })
        .then(() => {
